...
def test_to_openai_response_with_empty_predictions():
    embedding_content = {"predictions": []}
    model = "empty-model"
    expected = {"data": [], "model": "empty-model", "object": "list", "usage": {"total_tokens": 0}}
    assert to_openai_response(embedding_content, model) == expected
...
def test_to_openai_response_with_missing_predictions_key():
    embedding_content = {}
    model = "default-model"
    expected = {"data": [], "model": "default-model", "object": "list", "usage": {"total_tokens": 0}}
    assert to_openai_response(embedding_content, model) == expected


def test_to_openai_response_with_single_prediction():
    embedding_content = {"predictions": [{"embeddings": {"values": [1.0, 2.0, 3.0], "statistics": {"token_count": 5}}}]}
    model = "single-model"
    expected = {
        "data": [{"embedding": [1.0, 2.0, 3.0], "index": 0, "object": "embedding"}],
        "model": "single-model",
        "object": "list",
...
def test_to_openai_response_with_nonstandard_embedding_key():
    ...
    embedding_content = {"predictions": [{"not_embeddings": {"values": [1, 2, 3]}}]}
    model = "bad-model"
    try:
